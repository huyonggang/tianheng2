package com.tianheng.client.jpush;
...
import com.tianheng.client.util.Logger;
...
public class TagAliasOperatorHelper {
    ...
    private boolean RetryActionIfNeeded(int errorCode,TagAliasBean tagAliasBean){
        ...
        if(errorCode == 6002 || errorCode == 6014){
            ...
            if(tagAliasBean!=null){
                ...
                String Loggers =getRetryStr(tagAliasBean.isAliasAction, tagAliasBean.action,errorCode);
                return true;
            }
        }
        return false;
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        if(tagAliasBean == null){
//            ToastUtil.showShort(context,"获取缓存记录失败");
            return;
        }
        if(jPushMessage.getErrorCode() == 0){
            Logger.i(TAG,"action - modify tag Success,sequence:"+sequence);
            setActionCache.remove(sequence);
            String Loggers = getActionStr(tagAliasBean.action)+" tags success";
            Logger.i(TAG,Loggers);
            ToastUtil.showShort(context,Loggers);
        }else{
            String Loggers = "Failed to " + getActionStr(tagAliasBean.action)+" tags";
            if(jPushMessage.getErrorCode() == 6018){
                //tag数量超过限制,需要先清除一部分再add
                Loggers += ", tags is exceed limit need to clean";
            }
            Loggers += ", errorCode:" + jPushMessage.getErrorCode();
            Logger.e(TAG, Loggers);
...
//            ToastUtil.showShort(context,"获取缓存记录失败");
        ...
        if(jPushMessage.getErrorCode() == 0){
            ...
            setActionCache.remove(sequence);
            String Loggers = getActionStr(tagAliasBean.action)+" tag "+jPushMessage.getCheckTag() + " bind state success,state:"+jPushMessage.getTagCheckStateResult();
            Logger.i(TAG,Loggers);
            ToastUtil.showShort(context,Loggers);

        }else{
            String Loggers = "Failed to " + getActionStr(tagAliasBean.action)+" tags, errorCode:" + jPushMessage.getErrorCode();
            Logger.e(TAG, Loggers);
            if(!RetryActionIfNeeded(jPushMessage.getErrorCode(),tagAliasBean)) {
                ToastUtil.showShort(context,Loggers);
            }
...
//            ToastUtil.showShort(context,"获取缓存记录失败");
        ...
        if(jPushMessage.getErrorCode() == 0){
            ...
            setActionCache.remove(sequence);
            String Loggers = getActionStr(tagAliasBean.action)+" alias success";
            Logger.i(TAG,Loggers);
            ToastUtil.showShort(context,Loggers);
        }else{
            String Loggers = "Failed to " + getActionStr(tagAliasBean.action)+" alias, errorCode:" + jPushMessage.getErrorCode();
            Logger.e(TAG, Loggers);
            if(!RetryActionIfNeeded(jPushMessage.getErrorCode(),tagAliasBean)) {
                ToastUtil.showShort(context,Loggers);
            }
    ...
    public void onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        if(jPushMessage.getErrorCode() == 0){
            ...
            String Loggers = "Failed to set mobile number, errorCode:" + jPushMessage.getErrorCode();
            Logger.e(TAG, Loggers);
