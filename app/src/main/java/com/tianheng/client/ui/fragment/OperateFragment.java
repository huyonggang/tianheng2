...
import com.tianheng.client.util.QRCodeUtil;
...
//        boxStatuses.clear();
    ...
    private void initQRCode() {
        ...
        Bitmap mBitmap = QRCodeUtil.createQRCodeBitmap(imei, 300, 300);
    ...
    public void onEvent(GoodsStatusEvent event) {
        ...
        for (int i = 0; i < goods.size(); i++) {
            if (goods.get(i) == 0) {
                ...
                boxStatuses.get(i).setEmpty(false);
            }
    ...
    public void onEvent(DoorStatusEvent event) {
        ...
        if (!isOpen) {
            if (status == 3 && event.iLockId == mExchangeBean.getEmptyBoxNumber()) {
                ...
                closeDialog();
                if (inputBms != null) {
                    mPresenter.closeOld(inputBms.code, mExchangeBean.getEmptyBoxNumber());
                }
            ...
            } else if (status == 21 && event.iLockId == mExchangeBean.getExchangeBoxNumber()) {
                status = 22;
                if (disposable != null) {
                    disposable.dispose();
                }
    ...
    public void onEvent(GoodStatusEvent event) {
        ...
        if (isGoods) {//有物体
            ...
            } else if (status == 22 && lockId == mExchangeBean.getExchangeBoxNumber()) {
                if (disposable != null) {
                    disposable.dispose();
                }
                mCabinetManager.openDoor(0, mExchangeBean.getExchangeBoxNumber());
                ToastUtil.showShort(mContext, "请取出电池");
                status = 21;//网络请求失败
                if (disposable == null || disposable.isDisposed()) {
            ...
            if (status == 2 && lockId == mExchangeBean.getEmptyBoxNumber()) {
                ...
                closeDialog();
                mPresenter.closeNew(mExchangeBean.getExchangeBoxNumber(), mExchangeBean.getLeaseBatteryNumber());
            } else if (status == -1) {
    ...
    @Override
    public void closeOldFail(int box) {
